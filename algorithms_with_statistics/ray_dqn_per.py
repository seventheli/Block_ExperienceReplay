...
from ray.rllib.algorithms.dqn.dqn import update_priorities_in_replay_buffer


class DQNWithTimeCostLog(DQN):
    def __init__(self, config, env, logger_creator=None, **kwargs, ):
        super().__init__(config=config, env=env, logger_creator=logger_creator, **kwargs)
        self.time_usage = {
            "store": 0,
            "sample": 0,
            "train": 0,
    ...
    def _init(self, config: AlgorithmConfigDict, env_creator: EnvCreator) -> None:
        # config["replay_buffer_config"] = None
        # config["_disable_execution_plan_api"] = True
        super(DQNWithTimeCostLog, self)._init(config, env_creator)
