...
def train(algorithm, settings, checkpoint_path, mlflow_client, mlflow_run, yml_path):
    # list to save data
    episode_rewards = []
    episode_lengths = []
    ...
    for i in tqdm.tqdm(range(1, 10000)):
        ...
        if train_results["info"]["learner"].get("default_policy", None) is not None:
            ...
            if trained == 0:
                trained = int(train_results["info"]["learner"]["default_policy"]["step_count"])
            try:
            ...
            if i > 0 and i % settings.control.save == 0:
                try:
                    np.save(os.path.join(checkpoint_path, "episode_reward.npy"),
                            np.array([episode_rewards, episode_lengths]))
                    np.save(os.path.join(checkpoint_path, "eval_episode_reward.npy"),
                            np.array([eval_episode_rewards, eval_episode_lengths]))
                    mlflow_client.log_artifact(mlflow_run.info.run_id,
                                               os.path.join(checkpoint_path, "episode_reward.npy"))
                    mlflow_client.log_artifact(mlflow_run.info.run_id,
                                               os.path.join(checkpoint_path, "eval_episode_reward.npy"))
                    # save model
